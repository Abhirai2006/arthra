...
const composeHtml = (template: string, appHtml: string, head: HeadMeta, state: unknown, nonce: string) => {
  ...
  return template.replace("</body>", () => `<script nonce="${nonce}">window.__RQ_STATE__=${serialized}</script></body>`).replace("<!--app-head-->", () => buildHead(head)).replace("<!--app-html-->", () => appHtml);
};
...
export async function setupVite(app: Express, server: Server) {
  ...
  app.use("*", async (req, res, next) => {
    try {
      ...
      template = injectCspNonce(template, getCspNonce(res));
      ...
      res.status(head.notFound ? 404 : 200).set("Cache-Control", "no-cache").type("html").end(composeHtml(template, html, head, dehydratedState, getCspNonce(res)));
...
export async function renderStaticSsrPage(req: express.Request, res: express.Response) {
  ...
  try {
    const template = injectCspNonce(await fs.promises.readFile(templatePath, "utf-8"), getCspNonce(res)); const entry = path.resolve(import.meta.dirname, "server-ssr", "entry-server.js");
    ...
    const template = injectCspNonce(await fs.promises.readFile(templatePath, "utf-8"), getCspNonce(res)); const head = { title: "Arthra — Money with context", description: "A private Indian personal-finance workspace." };
    res.status(200).set("Cache-Control", "no-cache").type("html").end(composeHtml(template, "", head, {}, getCspNonce(res)));
  }
}
