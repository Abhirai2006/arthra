...
export function BudgetDialog({ open, onOpenChange, spaceId, monthKey, categories, initial }: { open: boolean; onOpenChange: (open: boolean) => void; spaceId: number; monthKey: string; categories: { id: number; name: string; kind: string }[]; initial?: { categoryId: number; amountPaise: number; alertAtPercent: number } | null }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="rounded-3xl border-border bg-card"><DialogHeader><DialogTitle>{initial ? "Adjust budget" : "Set a monthly budget"}</DialogTitle><DialogDescription>Choose one category and give it a useful boundary for this month.</DialogDescription></DialogHeader><div className="space-y-5 pt-3"><div><Label htmlFor="budget-category">Category</Label><select id="budget-category" value={categoryId} onChange={event => setCategoryId(event.target.value)} className="field-select mt-2"><option value="">Choose category</option>{expenses.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select></div><div className="form-grid"><div><Label htmlFor="budget-amount">Budget amount (₹)</Label><Input id="budget-amount" inputMode="decimal" value={amount} onChange={event => setAmount(event.target.value)} placeholder="0" className="mt-2" /></div><div><Label htmlFor="budget-alert">Alert at (%)</Label><Input id="budget-alert" inputMode="numeric" value={alertAtPercent} onChange={event => setAlertAtPercent(event.target.value)} className="mt-2" /></div></div><Button type="button" className="w-full rounded-xl py-6" disabled={save.isPending || !categoryId || !amount} onClick={() => save.mutate({ spaceId, monthKey, categoryId: Number(categoryId), amountPaise: Math.round(Number(amount) * 100), alertAtPercent: Number(alertAtPercent) || 80 })}>{save.isPending && <Loader2 className="mr-2 size-4 animate-spin" />}Save budget</Button></div></DialogContent></Dialog>;
}
