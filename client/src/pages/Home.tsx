...
import { ArrowRight, ArrowUpRight, BadgeIndianRupee, BrainCircuit, Check, ChevronRight, FileOutput, Fingerprint, Moon, Orbit, ReceiptText, ShieldCheck, Sparkles, Sun, UsersRound, WalletCards } from "lucide-react";
...
export default function Home() {
  ...
  return <main className="arthra-site">
    <section className="arthra-hero" ref={heroRef} onMouseMove={trackHero} style={pointerVariables}>
      <nav className="arthra-nav container" aria-label="Primary navigation"><a href="#top" className="brand-link"><BrandMark /></a><div className="arthra-nav__links" aria-label="Page sections"><a href="#why-arthra">Why Arthra</a><a href="#flows">How it works</a><a href="#trust">Privacy</a><a href="/demo">Demo</a></div><div className="arthra-nav__actions"><button className="theme-toggle" type="button" onClick={toggleTheme} aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}>{theme === "dark" ? <Sun size={16} /> : <Moon size={16} />}</button><button className="nav-login" type="button" onClick={handlePrimaryAction}>{isAuthenticated ? "Workspace" : "Sign in"}</button></div></nav>
      <nav className="mobile-quick-nav" aria-label="Mobile quick navigation"><a href="#why-arthra">Why</a><a href="#flows">Flow</a><a href="#trust">Private</a><a href="/demo">Demo</a><button type="button" onClick={handlePrimaryAction}>{isAuthenticated ? "Open" : "Start"}<ArrowRight size={15} /></button></nav>
    ...
    <section id="trust" className="trust-section container"><div className="trust-orb" aria-hidden="true"><span>₹</span></div><div className="trust-copy"><p className="eyebrow">Private from the first click</p><h2>Your financial data stays yours.</h2><p>The public site contains product information only. Authenticated routes protect transactions, reports, spaces, receipts, and insights; Expense Space roles and revocable report links keep sharing controlled.</p><div className="trust-points"><span><ShieldCheck size={15} /> Protected financial routes</span><span><UsersRound size={15} /> Owner, Editor, Viewer roles</span><span><FileOutput size={15} /> Revocable CA links</span></div><button className="button-primary button-primary--light" type="button" onClick={handlePrimaryAction}>{isAuthenticated ? "Open workspace" : "Sign in securely"}<ArrowRight size={17} /></button></div></section>
    <section className="product-showcase container" aria-labelledby="showcase-title"><div className="section-intro"><p className="eyebrow">Product showcase</p><h2 id="showcase-title">Six focused surfaces, one coherent money system.</h2></div><div className="showcase-grid"><a href="/demo#dashboard"><WalletCards size={18} /><strong>Dashboard</strong><span>Monthly clarity</span></a><a href="/demo#transactions"><ReceiptText size={18} /><strong>Transactions</strong><span>Details that travel</span></a><a href="/demo#budgets"><BadgeIndianRupee size={18} /><strong>Budgets</strong><span>Boundaries in view</span></a><a href="/demo#insights"><BrainCircuit size={18} /><strong>Analytics</strong><span>Calculated context</span></a><a href="/demo#spaces"><UsersRound size={18} /><strong>Expense Spaces</strong><span>Share with intent</span></a><a href="/demo#reports"><FileOutput size={18} /><strong>Reports</strong><span>Clean handoff</span></a></div></section>
    <footer className="arthra-footer container"><div><BrandMark /><p>Money, with a little more context.</p></div><div className="footer-links"><a href="#why-arthra">Features</a><a href="/demo">Demo</a><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="https://github.com/Abhirai2006/arthra" target="_blank" rel="noreferrer">GitHub</a><a href="#top">Back to top</a></div><p className="footer-meta">© 2026 Arthra. Built for deliberate money habits.</p></footer>
  </main>;
}
