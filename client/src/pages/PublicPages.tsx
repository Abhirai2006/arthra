...
export function WaitlistPage() {
  ...
  return <PublicShell current="Waitlist" eyebrow="Early access" title="Stay close to what Arthra builds next." intro="Join the optional product-update list for new workspace capabilities and selected research invitations."><section className="public-form-card"><div className="public-form-card__cta"><Sparkles size={20} /><div><h2>One focused update at a time.</h2><p>Use your email only if you want to hear about meaningful Arthra releases. You can withdraw consent whenever you choose; otherwise, inactive Waitlist records are removed after 12 months.</p></div></div><form onSubmit={event => { event.preventDefault(); if (consent) join.mutate({ email, consent, source: "waitlist", website }); }}><div className="public-honeypot" aria-hidden="true"><Label htmlFor="waitlist-website">Website</Label><Input id="waitlist-website" tabIndex={-1} autoComplete="off" value={website} onChange={event => setWebsite(event.target.value)} /></div><Label htmlFor="waitlist-email">Email address</Label><Input id="waitlist-email" type="email" autoComplete="email" required value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" /><label className="public-consent"><input type="checkbox" checked={consent} onChange={event => setConsent(event.target.checked)} required /> I agree to receive occasional Arthra product updates at this email.</label>{join.error && <p className="public-form-error" role="alert">{join.error.message}</p>}<Button type="submit" disabled={join.isPending || !consent}>{join.isPending ? "Joining…" : "Join the waitlist"}<ArrowRight size={16} /></Button></form></section></PublicShell>;
}
